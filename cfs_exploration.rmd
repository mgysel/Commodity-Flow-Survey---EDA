...
cfs_pairplot <- cfs_sample %>% select(QUARTER, SHIPMT_VALUE, SHIPMT_WGHT, 
                                      SHIPMT_DIST_ROUTED, TEMP_CNTL_YN, 
                                      EXPORT_YN, HAZMAT)
ggpairs(cfs_pairplot)
```
...
cfs.MODE_DIST <- cfs %>% group_by(MODE) %>% summarise(median=median(SHIPMT_DIST_ROUTED), mean=mean(SHIPMT_DIST_ROUTED))
...
shipments have low shipment weights. At approximately 50 pound shipment weight,
...
ggplot(data=cfs, aes(x=SHIPMT_DIST_GC, y=dist_ratio)) +
  geom_line(aes(color=cfs$EXPORT_CNTRY)) +
